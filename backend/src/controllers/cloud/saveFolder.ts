...
type save_folder = (folder: Folder<cloud>, userId: number) => void;
type saveType = (
...
const save: saveType = async (folder, folderName, userId) => {
  ...
  const folderDB = await Cloud.create({
    ...
    type: "folder",
    name: folderName,
    size: Math.round(folderData.size) + "KB",
  }).then((val) => val.get({ plain: true }));

  folderData.fileId.map((id) => {
    Folder_Cloud.build({
      folderId: folderDB.id,
      cloudId: id,
    }).save();
  });
  return { fileId: folderDB.id, size: Math.round(folderData.size) + "KB" };
};
...
const saveFolder: save_folder = async (folder, userId) => {
  const files = await Promise.all(moveStruct(folder, userId));
  save(files, folder.folderName, userId);
};
